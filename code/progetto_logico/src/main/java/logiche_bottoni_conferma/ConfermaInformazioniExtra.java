...
public class ConfermaInformazioniExtra {
	...
	/**
	 * Classe che prende i dati passati da "ConfermaInformazioniExtra",
		...
		frame.confermaButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Integer risultato = contesto.select(Diariamed.DIARIAMED.CODICE).from(Diariamed.DIARIAMED).where(Diariamed.DIARIAMED.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariamed.DIARIAMED.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount()))).orderBy(Diariamed.DIARIAMED.CODICE.desc()).limit(1).fetchOneInto(int.class);
					int ultimoCodice;
					if (risultato == null) {
						ultimoCodice = 0;
					}
					...
					SwingUtilities.invokeLater(new Runnable() {
					    @Override
					    public void run() {
							if (InserimentoJooq.getIstanza().diariaMed(ultimoCodice+1,modello.modelloGestorePaziente.getCodice(),modello.modelloGestorePaziente.getCount(),modello.modelloGestoreUtente.getCodiceUtente(),storico,motivo,reparto,farmaci,LocalDate.now(),LocalTime.now().withNano(0),informazioni)==1) {
