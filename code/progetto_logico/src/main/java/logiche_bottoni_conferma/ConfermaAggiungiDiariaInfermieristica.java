			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (!frame.farmacoTextArea.getText().isBlank()) {
						...
						Integer risultato = contesto.select(Diariainf.DIARIAINF.CODICE).from(Diariainf.DIARIAINF).where(Diariainf.DIARIAINF.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariainf.DIARIAINF.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount()))).orderBy(Diariainf.DIARIAINF.CODICE.desc()).limit(1).fetchOneInto(int.class);
						int ultimoCodice;
						if (risultato == null) {
							ultimoCodice = 0;
						}
						...
						if (InserimentoJooq.getIstanza().diariaInf(ultimoCodice+1,modello.modelloGestorePaziente.getCodice(),modello.modelloGestorePaziente.getCount(),modello.modelloGestoreUtente.getCodiceUtente(),LocalDate.now(),LocalTime.now().withNano(0),note,importante,farmaco) == 1) {
						}
						else {
