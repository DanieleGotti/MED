			...
			if(modello.modelloGestorePaziente.qualcunoSelezionato()) {
				if (modello.modelloGestoreUtente.getMansioneUtente().equals("Operatore")) {
					...
					try {
						...
						String repartoConsigliato = contesto.select(Diariamed.DIARIAMED.REPARTO_CONSIGLIATO).from(Diariamed.DIARIAMED).where(Diariamed.DIARIAMED.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariamed.DIARIAMED.CODICE.eq(1)).and(Diariamed.DIARIAMED.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount()))).fetchOneInto(String.class);
