...
public class LogicaBottoneModificaDiariaMedica extends LogicaBottone{
	 ...
	 * se tutto è in regola delega l'operazione di modifica alla classe logica "ConfermaModificaDiariaMedica", 
				...
				if(modello.modelloGestorePaziente.qualcunoSelezionato()) {
					if (modello.modelloGestoreUtente.getMansioneUtente().equals("Medico")) {
						...
						try {
							...
							String motivo = contesto.select(Diariamed.DIARIAMED.MOTIVO).from(Diariamed.DIARIAMED).where(Diariamed.DIARIAMED.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariamed.DIARIAMED.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount())),Diariamed.DIARIAMED.CODICE.eq(1)).fetchOneInto(String.class);
							...
							String farmaci = contesto.select(Diariamed.DIARIAMED.FARMACI).from(Diariamed.DIARIAMED).where(Diariamed.DIARIAMED.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariamed.DIARIAMED.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount())),Diariamed.DIARIAMED.CODICE.eq(1)).fetchOneInto(String.class);
							String informazioni = contesto.select(Diariamed.DIARIAMED.ALLERGIE).from(Diariamed.DIARIAMED).where(Diariamed.DIARIAMED.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariamed.DIARIAMED.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount())),Diariamed.DIARIAMED.CODICE.eq(1)).fetchOneInto(String.class);
							String codice = contesto.select(Diariamed.DIARIAMED.CODICE_MEDICO).from(Diariamed.DIARIAMED).where(Diariamed.DIARIAMED.CODICE_DEGENTE.eq(modello.modelloGestorePaziente.getCodice()).and(Diariamed.DIARIAMED.COUNT_DEGENTE.eq(modello.modelloGestorePaziente.getCount())),Diariamed.DIARIAMED.CODICE.eq(1)).fetchOneInto(String.class);
							String cognome = contesto.select(Personale.PERSONALE.COGNOME).from(Personale.PERSONALE).where(Personale.PERSONALE.CODICE.eq(codice)).fetchOneInto(String.class);
							frame.farmaciTextArea.setText(farmaci);
							frame.motivoTextField.setText(motivo);
							frame.storicoTextField.setText(storico);
							frame.modificatoLabel.setText("Ultima modifica: " + cognome + " (" + codice + ")");
